...
app.config['CDN_BASE'] = 'https://cdn.jsdelivr.net/npm'
...
class LottoForm(FlaskForm):
    issue_code = IntegerField('期号')
    red1 = IntegerField('红球1')
    red2 = IntegerField('红球2')
    red3 = IntegerField('红球3')
    red4 = IntegerField('红球4')
    red5 = IntegerField('红球5')
    red6 = IntegerField('红球6')
    blue = IntegerField('蓝球')
    sunday = IntegerField('快乐星期天')
    pool_amount = IntegerField('奖池奖金(元)')
    first_count = IntegerField('一等奖注数')
    first_amount = IntegerField('一等奖奖金(元)')
    second_count = IntegerField('二等奖注数')
    second_amount = IntegerField('二等奖奖金(元)')
    total_amount = IntegerField('总投注额(元)')
    draw_date = DateField('开奖日期')
...
@app.route('/lotto_form', methods=['GET', 'POST'])
def lotto_form():
    form = LottoForm()
    if form.validate_on_submit():
        flash('Form validated!')
