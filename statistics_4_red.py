...
# 统计4个RED在红球区出现的次数
def count_4_in_csv(data, a, b, c, d):
    # 判断RED是否在红区
    if a not in RED_NUMBERS or b not in RED_NUMBERS or c not in RED_NUMBERS or d not in RED_NUMBERS:
        return 0
    ...
    for line in data.itertuples():
        ...
        # print(arr)
        # 如果数字都在数组里，计数器+1
        if a in arr and b in arr and c in arr and d in arr:
    ...
    while a <= MAX_RED_NUM:
        ...
        while b <= MAX_RED_NUM:
            ...
            while c <= MAX_RED_NUM:
                d = c + 1
                while d <= MAX_RED_NUM:
                    count, ids = count_4_in_csv(data, a, b, c, d)
                    ...
                    for id in ids:
                        ...
                        ids_str += id + " "
                    print_result(a, b, c, d, count, ids_str)
                    row = [a, b, c, d, count, ids_str]
                    arr.append(row)
...
def write_csv(arr):
    ...
    cw = csv.writer(f)
    cw.writerow(["RED1", "RED2", "RED3", "RED4", "OCCURRENCE", "ISSUE"])
